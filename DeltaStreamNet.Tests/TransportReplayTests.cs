...
public class TransportReplayTests
{
    ...
    public void Replay_PartialFromOffset_ConsumerCatchesUp()
    {
        ...
        for (var i = 1; i <= 10; i++)
        {
            ...
            transport.PublishSilent(WireCodec.EncodeDeltaFrame(delta));
        }
    ...
    public void Redeliver_SingleDuplicate_Idempotent()
    {
        ...
        transport.PublishSilent(WireCodec.EncodeDeltaFrame(delta1)); // [1]
        var delta2 = encoder.EncodeChanges(Tick(502m));
        transport.PublishSilent(WireCodec.EncodeDeltaFrame(delta2)); // [2]
